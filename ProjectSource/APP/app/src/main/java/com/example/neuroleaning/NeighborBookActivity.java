...
public class NeighborBookActivity extends AppCompatActivity {
    ...
    // TODO: 테스트용으로 바꿔가며 확인할 step 값 (1~9)
    private int step = 8;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        all.add(new Neighbor(R.drawable.neighbor1, "편의점 직원 배수연",
                "첫 번째로 만난 배수연양! 나이는 22세, 활발한 성격입니다."));
        all.add(new Neighbor(R.drawable.neighbor2, "햄버거 가게 직원 조재영",
                "두 번째로 만난 조재영양! 나이는 23세, 친절하게 키오스크 사용법을 알려준 대학생입니다. 소프트웨어를 전공해요!"));
        all.add(new Neighbor(R.drawable.neighbor3, "친절한 옆집 이웃 이예솔",
                "세 번째로 만난 이예솔양, 게임 광으로, 마인크래프트 게임과 가위바위보 게임을 좋아해요!"));
        all.add(new Neighbor(R.drawable.neighbor4, "공원에서 만난 김다민",
                "네 번째로 만난 김다민양, 애완견 토리를 12년째 키우고 있는 견주입니다!"));
        all.add(new Neighbor(R.drawable.neighbor5, "옆집 아이 마루",
                "다섯 번째로 만난 마루양, 소문으로는 강아지였다가 사람이 됐다고 하는데, 진실은 아무도 몰라요!"));
        all.add(new Neighbor(R.drawable.neighbor6, "슈퍼 직원 정윤",
                "여섯 번째로 만난 정윤양, 친절한 옆집 이웃 이예솔이 가장 좋아하는 동생으로, 무척 귀엽고 엉뚱합니다."));
        all.add(new Neighbor(R.drawable.neighbor7, "일본인 유학생 야마다",
                "일곱 번째로 만난 야마다군, 일본인 고등학생으로, 잠깐 한국에 놀러왔어요! 게임을 무척 잘하고, 잘생겼어요."));
        all.add(new Neighbor(R.drawable.neighbor8, "버스 정류장에서 만난 정새연",
                "여덟 번째로 만난 새연양, 기억력이 매우 좋으며, 피씨방을 좋아해 매일 버스를 타고 방문합니다."));
        all.add(new Neighbor(R.drawable.neighbor9, "사랑하는 손녀 김토리",
        ...
        for (int i = 0; i < all.size(); i++) {
            ...
            if (i < step) {
                // 🔓 UNLOCKED
                Neighbor n = all.get(i);
                iv.setImageResource(n.imageRes);
                name.setText(n.name);
                desc.setText(n.desc);
                ...
                // 🔒 LOCKED
                iv.setImageResource(R.drawable.ic_lock);       // 잠금 아이콘 필요
                name.setText("");
                desc.setText("");
                play.setText("잠겨있음");
